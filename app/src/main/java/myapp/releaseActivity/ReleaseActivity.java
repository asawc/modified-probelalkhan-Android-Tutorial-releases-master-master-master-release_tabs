            ...
            public void onResponse(Call<ResponseContainer<List<Release>>> call,
                ...
                for(Release rel : mReleasesList)
                    mReleaseViews.add(new ReleaseView(rel.getId(),
                            ...
                            rel.getProductsRelease().get(rel.getId()).getProduct().getName(),
                            rel.getProductsRelease().get(rel.getId()).getReqQuantity()*/));
            }
        });
    }
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    mReleaseViews.add(new ReleaseView(rel.getId(),
                            ...
                            rel.getEmployee().getSurname(),
                            rel.getEmployee().getName()/*,
                            rel.getProductsRelease().get(i).getProduct().getName(),
                            rel.getProductsRelease().get(i).getReqQuantity()*/));
...
*/
    ...
    public class ReleaseAdapter extends RecyclerView.Adapter<ReleaseAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
            ...
            holder.getEmployeeNameTextView().setText(mReleaseView.get(position).getEmployeeName());
        }
